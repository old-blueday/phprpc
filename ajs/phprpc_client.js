...
/* Another PHPRPC Client for browser JavaScript (using Flash)
 *
...
var PHPRPC_Client = (function () {
    ...
    return (function() {
        ...
        function PHPRPC_Client(serverURL, functions) {
            ...
            var m_ready    = false;
            var m_preID    = 'ajs' + Math.floor((new Date()).getTime() * Math.random());
            ...
            function useService(onready) {
                if (m_ajax) {
                    ...
                    FlashRequest.get(m_serverURL, 'phprpc_encode=false', m_username, m_password, function (data) {
                        var id = createID();
                        createDataObject(data, id);
                        setFunctions(m_php.unserialize(m_dataObject[id].phprpc_functions), onready);
                        deleteDataObject(id);
                    });
                }
            ...
            function keyExchange() {
                ...
                if (m_key == null && m_encryptMode > 0) {
                    ...
                    if (m_ajax) {
                        ...
                        FlashRequest.get(m_serverURL, 'phprpc_encrypt=true&phprpc_encode=false&phprpc_keylen=' + m_keyLength, m_username, m_password, function (data) {
                            var id = createID();
                            createDataObject(data, id);
                            keyExchange2(id);
                            deleteDataObject(id);
            ...
            function keyExchange2(id) {
                ...
                if (typeof(data.phprpc_encrypt) == 'undefined') {
                    ...
                    if (m_ajax) {
                        ...
                        FlashRequest.get(m_serverURL, 'phprpc_encode=false&phprpc_encrypt=' + encrypt, m_username, m_password, function(data) {
                            m_keyExchanging = false;
                            keyExchanged();
                        });
                    }
            ...
            function call(id, func, args) {
                ...
                if (m_ajax) {
                    ...
                    FlashRequest.post(m_serverURL, request.replace(/\+/g, '%2B'), m_username, m_password, function (data) {
                        createDataObject(data, id);
                        getResult(id, args, ref, encrypt, callback);
                        deleteDataObject(id);
                        deleteReqHeap(id);
