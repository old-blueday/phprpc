...
uses SysUtils, Variants, VarUtils;
...
begin
  ...
  repeat
    ...
    begin
      ...
      begin
        Result := MethodInfo;
        if {$IFDEF DELPHI2009_UP}SamePropTypeName{$ELSE}SameText{$ENDIF}(Result^.Name, MethodName) then
          Exit;
        Inc(Integer(MethodInfo), PMethodInfoHeader(MethodInfo)^.Len);
        Dec(Count);
      end;
...
var
  ...
  ParamBytes:    Integer;
  Size:          Integer;
  Frame:         {$IFDEF DELPHI2009_UP}PByte{$ELSE}PChar{$ENDIF};
  ResultParam:   Pointer;
...
begin
  ...
  MethodName := {$IFDEF DELPHI2009_UP}UTF8ToString({$ENDIF} PMethodInfoHeader(MethodInfo)^.Name{$IFDEF DELPHI2009_UP}){$ENDIF};
...
{$endif}
  ...
  begin
    ...
    if PushData = nil then
      if (Param^.ParamType^.Kind = tkClass) and {$IFDEF DELPHI2009_UP}SamePropTypeName{$ELSE}SameText{$ENDIF}(Param^.Name, 'SELF') then
...
var
  P: {$IFDEF DELPHI2009_UP}PByte{$ELSE}PChar{$ENDIF};
  I: Integer;
begin
  ...
  begin
    Inc(P, 1 + {$IFNDEF DELPHI2009_UP}Byte{$ENDIF}(P[1]) + 1);
    Inc(P, {$IFNDEF DELPHI2009_UP}Byte{$ENDIF}(P[0]) + 1);
  end;
  if TypeData^.MethodKind = mkFunction then
    // Skip return type name and info
    Inc(P, {$IFNDEF DELPHI2009_UP}Byte{$ENDIF}(P[0]) + 1 + 4);
  Result := P;
end;
